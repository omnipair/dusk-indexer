...
import {readArchivedQuoteHistory,ARCHIVED_QUOTE_REVISION} from '../services/duskArchivedQuoteHistory';
...
after(()=>pool.end());
const root=resolve(__dirname,'../../../protocol/archive',ARCHIVED_QUOTE_REVISION),pin=loadProtocolAt(root);
...
const envelopes=fixtures('archived-quote-deployments.json');

test('archived candles retain their original evidence and remain excluded from active history',async()=>{
  ...
  try {
    ...
    for(const raw of fixtures('archived-quote-captures.json')) {
    ...
    const query={market:'7Rjrf8i81hZihsuFdPfzTaP6SiQ3JEmQs7Hfg7YNjkNm',side:'base' as const,since:'2026-09-14T00:00:00Z',until:'2026-09-17T18:00:00Z',resolutionSeconds:900,deploymentIdentitySha256:envelopes[0].deploymentIdentitySha256};
    const archived=await readArchivedQuoteHistory(client,query,ARCHIVED_QUOTE_REVISION);
