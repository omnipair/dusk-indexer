...
async function main() {
  ...
  const intervalMs = Number(process.env.DUSK_PRICE_INTERVAL_MS ?? 5_000);
  ...
  do {
    ...
    // Start-to-start cadence, without overlapping captures or catch-up bursts.
    // Slow RPC work consumes the period instead of adding another full delay.
    const remainingMs = Math.max(0, intervalMs - (Date.now() - startedAt));
    for (let elapsed = 0; elapsed<remainingMs && !stopped; elapsed+=500)
      await new Promise((resolve) => setTimeout(resolve,Math.min(500,remainingMs-elapsed)));
  } while (!stopped);
}
