...
An empty new-release history creates a cursor at the first deployment slot with no fabricated last signature. The stream advances it to the last persisted slot, and a 15-second heartbeat shows liveness only; it does not prove gap-free coverage. There is no backfill: transactions confirmed while the stream is down stay missing unless replayed.
